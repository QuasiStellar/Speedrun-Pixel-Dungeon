...
import com.quasistellar.speedrunpixeldungeon.ui.CheckBox;
import com.quasistellar.speedrunpixeldungeon.ui.GameLog;
import com.quasistellar.speedrunpixeldungeon.ui.Icons;
import com.quasistellar.speedrunpixeldungeon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		SpeedrunPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							SpeedrunPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						SpeedrunPixelDungeon.scene().addToFront(new WndKeyBindings());
					}
				};
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						SpeedrunPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
