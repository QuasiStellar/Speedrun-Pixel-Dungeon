...
import com.quasistellar.speedrunpixeldungeon.services.news.News;
import com.quasistellar.speedrunpixeldungeon.services.updates.AvailableUpdateData;
import com.quasistellar.speedrunpixeldungeon.services.updates.Updates;
import com.quasistellar.speedrunpixeldungeon.sprites.CharSprite;
import com.quasistellar.speedrunpixeldungeon.sprites.RogueSprite;
...
import com.quasistellar.speedrunpixeldungeon.windows.WndSettings;
...
//		placeTorch(title.x + title.width - 22, title.y + 46);
		...
		placeRogue(title.x + 85, title.y + 18);
		...
		StyledButton btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					SpeedrunPixelDungeon.switchScene(HeroSelectScene.class);
				} else {
					SpeedrunPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				SpeedrunPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "badges")){
			@Override
			protected void onClick() {
				SpeedrunPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		...
		StyledButton btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				SpeedrunPixelDungeon.switchScene( AboutScene.class );
			}
		};
		btnAbout.icon(Icons.get(Icons.WAND));
		add(btnAbout);
	...
	private void placeRogue(float x, float y) {
		RogueSprite rogue = new RogueSprite();
		rogue.x = x;
		rogue.y = y;
		add(rogue);
	}
...
//	}
	...
	private static class NewsButton extends StyledButton {
		...
		protected void onClick() {
			super.onClick();
			SpeedrunPixelDungeon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class ChangesButton extends StyledButton {
		...
		protected void onClick() {
			if (Updates.isInstallable()){
				...
				SpeedrunPixelDungeon.scene().addToFront(new WndOptions(
					...
					protected void onSelect(int index) {
						if (index == 0) {
							...
							SpeedrunPixelDungeon.switchNoFade( ChangesScene.class );
						}
					}
				});
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			SpeedrunPixelDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			SpeedrunPixelDungeon.switchNoFade(SupporterScene.class);
		}
	}
}
