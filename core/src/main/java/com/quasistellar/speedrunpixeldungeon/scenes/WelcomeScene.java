...
import com.quasistellar.speedrunpixeldungeon.effects.Fireball;
...
public class WelcomeScene extends PixelScene {
	...
	public void create() {
		...
		if (SpeedrunPixelDungeon.versionCode == previousVersion) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0){
					SPDSettings.version(SpeedrunPixelDungeon.versionCode);
		...
		if (previousVersion != 0){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					SpeedrunPixelDungeon.switchScene(ChangesScene.class);
				}
			};
			...
			okay.text(Messages.get(this, "start"));
		...
		} else if (previousVersion <= SpeedrunPixelDungeon.versionCode) {
