...
import com.quasistellar.speedrunpixeldungeon.items.LostBackpack;
...
public class InterlevelScene extends PixelScene {
	...
	//instant fade when descending to the 1st floor
	private static final float INSTANT_FADE = 0f;
	...
	public void update() {
		...
		case STATIC:
			if (error != null) {
				...
				SpeedrunPixelDungeon.reportException(
	...
	private void descend() throws IOException {
		...
		if (Dungeon.depth >= Statistics.deepestFloor) {
			if (Dungeon.depth >= 1) {
				Dungeon.splits[Dungeon.depth - 1] = Dungeon.currentTime - Dungeon.lastSplitTime;
				Dungeon.lastSplitTime = Dungeon.currentTime;
				Dungeon.goalSplitTime += ((double)SPDSettings.personalBest(Dungeon.category) / (double)SPDSettings.averageRunTime(Dungeon.category)) * SPDSettings.averageSplitTime(Dungeon.category, Dungeon.depth + 1);
			}
	...
	private void fall() throws IOException {
		...
		if (Dungeon.depth >= Statistics.deepestFloor) {
			if (Dungeon.depth >= 1) {
				Dungeon.splits[Dungeon.depth - 1] = Dungeon.currentTime - Dungeon.lastSplitTime;
				Dungeon.lastSplitTime = Dungeon.currentTime;
				GLog.h(Long.toString(Dungeon.lastSplitTime));
				Dungeon.goalSplitTime += ((double)SPDSettings.personalBest(Dungeon.category) / (double)SPDSettings.averageRunTime(Dungeon.category)) * SPDSettings.averageSplitTime(Dungeon.category, Dungeon.depth + 1);
			}
			level = Dungeon.newLevel();
		} else {
