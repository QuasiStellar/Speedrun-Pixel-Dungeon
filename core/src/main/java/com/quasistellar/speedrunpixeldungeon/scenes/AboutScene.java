...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		//*** Speedrun Pixel Dungeon Credits ***

		String qsrLink = "https://quasistellar.xyz";

		CreditsBlock qsr = new CreditsBlock(true, Window.BLUE,
				"Speedrun Pixel Dungeon",
				Icons.WAND.get(),
				"Developed by: _QuasiStellar_\nBased on Shattered Pixel Dungeon's open source",
				"quasistellar.xyz",
				qsrLink);
		qsr.setRect((w - colWidth)/2f, 10, 120, 0);
		content.add(qsr);
		...
		CreditsBlock shpx = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				shpxLink);
		if (landscape()){
			shpx.setRect((w - fullWidth)/2f - 6, qsr.bottom() + 14, 120, 0);
		} else {
			shpx.setRect((w - fullWidth)/2f, qsr.bottom() + 14, 120, 0);
		}
		content.add(shpx);

		addLine(shpx.top() - 10, content);
